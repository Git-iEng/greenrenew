...
def request_demo_view(request):
    ...
    subject = "New Enquirey for Junction Box from green renewable energy"
    text_body = (
    ...
    html_body = f"""
        <h2 style="margin:0 0 8px">New Enquirey for Junction Box from green renewable energy</h2>
...
def contact_section(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        # Email body
        subject = "New green renewable energy contact submission for Junction Box"
        text_body = "\n".join(
            [
...
def contact_block_submit(request):
    ...
    # --- Email notification ---
    subject = f"[Green Renewable Energy] Consulting request: {name}"
 
    text_body = "\n".join([
        "A new consulting request was submitted for green renewable energy:",
        f"Name: {name}",
        f"Email: {email}",
